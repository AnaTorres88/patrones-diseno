...
class LocationIcon implements Location {
    ...
    display(coordinates: {x: number, y: number}): void {
        console.log(`%cCoords: en coordenadas ${coordinates.x}, ${coordinates.y} con icono %c[${this.iconImage}]`, COLORS.blue);
    }
}

// Fabrica de Flywheights


class LocationFactory {
    private icons: Record<string, LocationIcon> = {};

    // Icono de: Escuela, Hospital
    getLocationIcon(type: string): LocationIcon {
        if(!this.icons[type]) {
            console.log(`%cSe creo un nuevo icono de tipo %c${type}`, COLORS.green, COLORS.white);
...
function main() {
    ...
    const locations: MapLocation[] = [
        ...
        new MapLocation(50, 60, factory.getLocationIcon('Parque')),
        new MapLocation(70, 80, factory.getLocationIcon('Pescadería')),
    new MapLocation(90, 100, factory.getLocationIcon('Escuela')),
                new MapLocation(30, 40, factory.getLocationIcon('Hospital')),
